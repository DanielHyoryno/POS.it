...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $sampleProducts = Product::with(['linkedItem:id,cost_price,base_unit','bomLines.item:id,cost_price,base_unit'])
        ...
        return view('admin.dashboard', compact(
            'totalItems','activeItems','lowStockCount','lowStockItems','recentMovements',
            'totalProducts','activeProducts','simpleCount','compositeCount',
            'compositeWithoutBom','simpleWithInactiveItem','productsNeedingCostData','sample',
            'nearExpiryLots'
        ));
    }
    ...
    public function salesSeries(Request $request)
    {
        ...
        if ($debug) {
            return response()->json([
                ...
                'debug'    => [
                    'range'     => $range,
                    'start_app' => $start->toDateTimeString(),
                    'now_app'   => $now->toDateTimeString(),
                    'start_utc' => $startUtc->toDateTimeString(),
                    'now_utc'   => $nowUtc->toDateTimeString(),
                    'sales_rows'=> $sales->count(),
                    'purch_rows'=> $purchases->count(),
                    'sales_sum' => (float)$sales->sum('total'),
                ],
            ]);
        }

        return response()->json(compact('labels','revenue','orders','expenses','profit'));
    }
}
