...
<style>
  ...
  .dash-backdrop { background: radial-gradient(80% 50% at 50% 0%, rgba(99,102,241,.08), rgba(14,165,233,.06), rgba(16,185,129,.05) 70%, transparent); }
...
<div class="dash-backdrop">
  <div class="container-xxl py-4 py-md-5">
    ...
    @php
      $cards = [
        ['label'=>'Total Items','value'=>$totalItems,'icon'=>'box'],
        ['label'=>'Active Items','value'=>$activeItems,'icon'=>'bolt'],
        ['label'=>'Low Stock','value'=>$lowStockCount,'danger'=> (bool) $lowStockCount,'icon'=>'warn'],
        ['label'=>'Total Products','value'=>$totalProducts,'icon'=>'layers'],
        ['label'=>'Active Products','value'=>$activeProducts,'icon'=>'check'],
    ...
    <section class="card card-quiet rounded-4 overflow-hidden my-4">
      <div class="card-header bg-body-tertiary d-flex align-items-center justify-content-between flex-wrap gap-2">
        ...
        <div class="btn-group" role="group" aria-label="Range">
          <button type="button" class="btn btn-sm btn-outline-secondary" data-range="1d">1d</button>
    ...
    <div class="row g-3 g-md-4">
      ...
      <div class="col-12 col-lg-4">
        <section class="card card-quiet rounded-4 h-100 overflow-hidden">
          <div class="card-header bg-body-tertiary"><strong>BOM Health</strong></div>
        ...
        <section class="card card-quiet rounded-4 h-100 overflow-hidden">
          <div class="card-header bg-body-tertiary d-flex align-items-center justify-content-between">
            <div>
              ...
              <div class="small text-secondary">Below threshold based on base unit.</div>
            </div>
            ...
            <table class="table table-sm align-middle table-sticky mb-0">
              <thead>
                <tr class="text-secondary text-uppercase small">
                  ...
                  <th scope="col" class="text-end">Threshold</th>
              ...
              <tbody class="table-group-divider">
                ...
                @empty
                  <tr>
                    <td colspan="4" class="text-center text-secondary py-4">No low stock 🎉</td>
                  </tr>
                @endforelse
              </tbody>
            </table>
      ...
      <div class="col-12 col-lg-4">
        <section class="card card-quiet rounded-4 h-100 overflow-hidden d-flex flex-column">
          <div class="card-header bg-body-tertiary">
            ...
            <div class="small text-secondary">Latest first. Click to load more.</div>
          </div>
          ...
          <div class="d-flex justify-content-between align-items-center p-2 border-top bg-body-tertiary">
            <div id="mv-loading" class="text-secondary small d-none">Loading…</div>
            <div class="d-flex gap-2">
              <button id="mv-more" class="btn btn-outline-secondary btn-sm">Load more</button>
              <span id="mv-done" class="text-muted small d-none">No more records 🎉</span>
            </div>
          </div>
        </section>
      </div>
    ...
    {{-- Near Expiry Lots --}}
    <section class="card card-quiet rounded-4 overflow-hidden my-4">
      <div class="card-header bg-body-tertiary"><strong>Expiring Soon (≤7d)</strong></div>
      <div class="list-group list-group-flush">
        @forelse($nearExpiryLots as $lot)
          @php
            $daysLeft = now()->diffInDays($lot->expiry_date,false);
            $toneKey = $daysLeft <= 2 ? 'danger' : ($daysLeft <= 4 ? 'warn' : 'caution');
        ...
        @empty
          <div class="list-group-item text-center text-secondary py-5">No near-expiry lots 🎉</div>
        @endforelse
      </div>
    </section>
      ...
      <div class="table-responsive">
        <table class="table table-sm align-middle mb-0">
          ...
          <tbody class="table-group-divider">
              ...
              <tr>
                <td>
                  ...
                  @if($p['simple_item_inactive'])
                    <span class="badge bg-warning-subtle text-warning-emphasis ms-2">Item Inactive</span>
                  @endif
                  @if($p['missing_cost'])
            ...
            @empty
              <tr><td colspan="7" class="text-center text-secondary py-4">No products yet.</td></tr>
            @endforelse
          </tbody>
        </table>
      </div>
...
(function(){
  ...
  function render(data) {
    const { labels, revenue, orders, expenses, profit } = data;
    ...
    chart = new Chart(ctx, {
      ...
      data: {
        labels,
        datasets: [
          { label: 'Revenue',  data: revenue,  tension: .35, borderWidth: 2, pointRadius: 0, borderColor: 'green', yAxisID: 'y' },
          { label: 'Expenses', data: expenses, tension: .35, borderWidth: 2, pointRadius: 0, borderColor: 'red',   yAxisID: 'y' },
          { label: 'Profit',   data: profit,   tension: .35, borderWidth: 2, pointRadius: 0, borderColor: 'blue',  yAxisID: 'y' },
          { label: 'Orders',   data: orders,   tension: .35, borderWidth: 2, borderDash: [4,4], pointRadius: 0, borderColor: 'gray', yAxisID: 'y1' },
        ]
      },
      options: {
        responsive: true,
        ...
        scales: {
          y:  { position: 'left',  ticks: { callback:v=>'Rp '+Number(v).toLocaleString('id-ID') } },
          y1: { position: 'right', ticks: { precision: 0 }, grid: { drawOnChartArea: false } },
  ...
  async function load(range='1m') {
    const res = await fetch(`${routeBase}?range=${range}`, { headers: { 'X-Requested-With': 'XMLHttpRequest' }});
