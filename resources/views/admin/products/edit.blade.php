...
<div class="max-w-3xl mx-auto p-6 space-y-4">
    <h1 class="text-2xl font-bold">{{ __('Edit Product') }}</h1>
    ...
    <form method="POST" action="{{ route('admin.products.update',$product) }}" enctype="multipart/form-data"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <select name="type" class="w-full border p-2 rounded" required>
                    <option value="simple" @selected(old('type',$product->type)==='simple')>{{ __('Simple') }}</option>
                    <option value="composite" @selected(old('type',$product->type)==='composite')>{{ __('Composite') }}
                    </option>
                </select>
            </div>
            ...
            <div>
                ...
                <input name="selling_price" type="number" step="0.01" min="0"
                    value="{{ old('selling_price',$product->selling_price) }}" class="w-full border p-2 rounded"
                    required>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <select name="category_id" class="w-full border p-2 rounded">
                    ...
                    <option value="{{ $cat->id }}" @selected(old('category_id',$product->
                        category_id)==$cat->id)>{{ $cat->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <img src="{{ Storage::url($product->image_path) }}" class="mt-2 h-20 w-20 rounded object-cover"
                    alt="{{ $product->name }}">
                @endif
            </div>
        </div>
        ...
        @if($product->isSimple())
        <div class="space-y-2">
            <div class="text-sm text-gray-600">{{ __('Linked Item & per sale qty (base unit)') }}</div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                ...
                <div>
                    <input name="per_sale_qty" type="number" step="0.001" min="0.001"
                        value="{{ old('per_sale_qty',$product->per_sale_qty) }}" class="w-full border p-2 rounded">
                </div>
...
<div class="text-right">
    ...
    <button class="px-4 py-2 bg-indigo-600 text-white rounded">{{ __('Update') }}</button>
</div>
</form>
...
<div class="bg-white rounded shadow p-4">
    <div class="flex items-center justify-between mb-2">
        <h2 class="font-semibold">{{ __('BOM Lines (quick view)') }}</h2>
