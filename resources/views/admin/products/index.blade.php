...
<div class="max-w-6xl mx-auto p-6">
    <div class="flex items-center justify-between mb-4">
        ...
        <a href="{{ route('admin.products.create') }}"
            class="px-3 py-2 bg-indigo-600 text-white rounded">{{ __('+ New Product') }}</a>
    </div>

    <form method="GET" class="mb-3 flex flex-wrap gap-2">
        <input name="search" value="{{ request('search') }}" class="border p-2 rounded"
    ...
    <div class="overflow-x-auto bg-white rounded shadow">
        <table class="min-w-full">
            <thead class="bg-gray-50">
                <tr>
                    <th class="text-left p-3">{{ __('Product') }}</th>
                    <th class="text-left p-3">{{ __('Type / Category') }}</th>
                    <th class="text-right p-3">{{ __('Price') }}</th>
                    <th class="text-center p-3">{{ __('Status') }}</th>
                    <th class="p-3"></th>
                </tr>
            </thead>
                ...
                <tr class="border-t">
                    <td class="p-3">
                        <div class="flex items-center gap-3">
                            @if($p->image_path)
                            <img src="{{ Storage::url($p->image_path) }}" class="h-10 w-10 rounded object-cover"
                                alt="{{ $p->name }}">
                            @else
                            <div class="h-10 w-10 rounded bg-gray-200 grid place-items-center text-gray-500 text-xs">IMG
                            </div>
                            @endif
                            <div>
                                <a class="text-blue-600 hover:underline font-medium"
                                    href="{{ route('admin.products.show',$p) }}">{{ $p->name }}</a>
                    ...
                    <td class="p-3 text-center">
                        <span
                            class="px-2 py-0.5 rounded {{ $p->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-700' }}">
                    ...
                    <td class="p-3 text-right space-x-2">
                        ...
                        <a href="{{ route('admin.products.bom.edit', $p) }}"
                            class="px-2 py-1 text-sm border rounded">{{ __('BOM') }}</a>
                        @endif
                        <a href="{{ route('admin.products.edit', $p) }}"
                            class="px-2 py-1 text-sm border rounded">{{ __('Edit') }}</a>
                        <form action="{{ route('admin.products.toggle', $p) }}" method="POST" class="inline">
                            @csrf
                            @method('PATCH')
                            <button
                                class="px-2 py-1 text-sm border rounded">{{ $p->is_active ? __('Deactivate') : __('Activate') }}</button>
                        </form>
                    </td>
                </tr>
                @empty
                <tr>
                    <td class="p-4 text-center text-gray-500" colspan="5">{{ __('No products yet.') }}</td>
                </tr>
                @endforelse
            </tbody>
        </table>
