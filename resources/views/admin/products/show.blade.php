...
<div class="max-w-3xl mx-auto p-6 space-y-4">
  ...
  <div class="flex items-start justify-between">
    <div class="flex gap-4">
      @if($product->image_path)
        <img src="{{ Storage::url($product->image_path) }}" class="h-20 w-20 rounded object-cover" alt="{{ $product->name }}">
      @else
        <div class="h-20 w-20 rounded bg-gray-200 grid place-items-center text-gray-500 text-xs">IMG</div>
      @endif
      <div>
        <h1 class="text-2xl font-bold">{{ $product->name }}</h1>
        @if($product->sku)<div class="text-gray-600">SKU: {{ $product->sku }}</div>@endif
        <div class="text-gray-600 capitalize">Type: {{ $product->type }}</div>
        @if($product->category) <div class="text-gray-600">Category: {{ $product->category->name }}</div> @endif
        <div>Price: <b>Rp {{ number_format($product->selling_price,2,',','.') }}</b></div>
        <div class="mt-2">
          Status:
          <span class="px-2 py-0.5 rounded {{ $product->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-700' }}">
    ...
    <div class="space-x-2">
      ...
      <form action="{{ route('admin.products.toggle', $product) }}" method="POST" class="inline">
        @csrf @method('PATCH')
        <button class="px-3 py-2 border rounded">{{ $product->is_active ? 'Deactivate' : 'Activate' }}</button>
      </form>
    </div>
  </div>
  ...
  <div class="bg-white rounded shadow p-4">
    <h2 class="font-semibold mb-2">Linked Item</h2>
    <div>{{ $product->linkedItem->name }} — per sale qty:
      <b>{{ rtrim(rtrim(number_format($product->per_sale_qty,3,'.',''), '0'), '.') }} {{ $product->linkedItem->base_unit }}</b>
    </div>
  </div>
  @endif
  ...
  @if($product->isComposite())
  <div class="bg-white rounded shadow p-4">
    <h2 class="font-semibold mb-2">BOM Lines</h2>
